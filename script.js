...
// ----- 4. DOM Елементи -----
let gameAreaElement, stickerImageElement, optionsContainerElement, timeLeftElement, currentScoreElement, resultAreaElement, finalScoreElement, playAgainButton, authSectionElement, loginButton, userStatusElement, userNicknameElement, logoutButton, difficultySelectionElement, loadingIndicator, errorMessageElement; // Змінено userEmailElement на userNicknameElement
...
// Елементи редагування нікнейму
let editNicknameButton, editNicknameForm, nicknameInputElement, cancelEditNicknameButton;
...
async function logout() {
    ...
    try { const { error } = await supabaseClient.auth.signOut(); if (error) { throw error; } console.log("supabaseClient.auth.signOut() successful."); }
...
function updateAuthStateUI(user) {
   ...
   hideNicknameEditForm(); // Завжди ховаємо форму при зміні стану

   if (user) {
       currentUser = user;
       userNicknameElement.textContent = currentUserProfile?.username || user.email || 'Loading...'; // Показуємо кешований нік або email
...
async function checkAndCreateUserProfile(user) {
   ...
   try {
       ...
       if (!profileData) {
           console.log(`Profile not found. Creating...`); const randomNickname = generateRandomNickname();
           ...
           currentUserProfile = insertedProfile; profileUsername = insertedProfile?.username;
           console.log(`Profile created with nickname: ${profileUsername}`);
       } else {
           currentUserProfile = profileData; profileUsername = profileData.username;
           console.log(`Profile exists. Username: ${profileUsername}`);
       }
       if (userNicknameElement) { userNicknameElement.textContent = profileUsername || user.email || 'User'; } // Update UI
   } catch (error) { console.error("checkAndCreateUserProfile Error:", error); showError(`Profile Error: ${error.message}`); if (userNicknameElement) userNicknameElement.textContent = user.email || 'User'; currentUserProfile = null; }
}
...
function setupAuthStateChangeListener() {
    ...
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
        console.log(`Auth Event: ${_event}`); const user = session?.user ?? null;
         if (initializeDOMElements()) { // Ensure elements are ready
            updateAuthStateUI(user); // Update basic UI state
            if (_event === 'SIGNED_IN' && user) { await checkAndCreateUserProfile(user); } // Create/Check profile, update nickname display
 ...
 async function checkInitialAuthState() {
     ...
     try { const { data: { session }, error } = await supabaseClient.auth.getSession(); if (error) throw error; currentUser = session?.user ?? null; console.log("Initial auth state checked.", currentUser ? `User: ${currentUser.id}` : "No user"); if (currentUser) { await checkAndCreateUserProfile(currentUser); } }
...
function showNicknameEditForm() {
    ...
    nicknameInputElement.value = userNicknameElement.textContent || ''; // Use current display name
    userNicknameElement.style.display = 'none';
    editNicknameButton.style.display = 'none'; // Hide edit button too
...
async function handleNicknameSave(event) {
    ...
    if (newNickname === currentUserProfile.username) { hideNicknameEditForm(); return; }
...
function initializeApp() {
    ...
    if (!initializeDOMElements()) { console.error("CRITICAL: Failed to initialize DOM elements."); return; } // Зупиняємось, якщо помилка
    setupAuthStateChangeListener(); // Налаштовуємо слухача
    updateAuthStateUI(currentUser); // Оновлюємо UI на основі currentUser
