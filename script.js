...
function initializeDOMElements() {
    ...
    if (!allFound) { console.error("initializeDOMElements: Not all elements found."); return false; }
    ...
    logoutButton.addEventListener('click', logout); // <-- Слухач кнопки Вийти
...
async function loginWithGoogle() {
    console.log("Login function CALLED!"); // Змінено текст логу для ясності
...
async function logout() {
    console.log("Logout function CALLED!"); // <--- ДОДАНО ЛОГ (переконуємось, що він є)
    if (!supabaseClient) { console.error("Logout Error: Supabase client missing!"); return showError("Supabase client not initialized."); }
    console.log("Attempting to sign out..."); // <-- ДОДАНО ЛОГ
    hideError();
    try {
        const { error } = await supabaseClient.auth.signOut();
        if (error) {
            console.error("Supabase signOut error object:", error); // <-- ДОДАНО ЛОГ помилки signOut
            throw error;
        }
        console.log("supabaseClient.auth.signOut() successful."); // <-- ДОДАНО ЛОГ успіху signOut
        // Не оновлюємо UI тут, чекаємо на onAuthStateChange
    } catch (error) {
        console.error("Logout function error:", error); // <-- ДОДАНО ЛОГ помилки в catch
...
async function checkAndCreateUserProfile(user) {
   ...
   try {
       ...
       if (!data) {
           ...
           const { data: insertedProfile, error: insertError } = await supabaseClient.from('profiles').insert({ id: user.id, username: randomNickname, updated_at: new Date() }).select('username').single(); // Додано select('username')
           if (insertError) throw insertError;
           console.log(`Profile created with nickname: ${insertedProfile?.username}`);
           if(userNicknameElement && insertedProfile) userNicknameElement.textContent = insertedProfile.username;
       } else {
           console.log(`Profile exists. Username: ${data.username}`);
           if(userNicknameElement && data.username) userNicknameElement.textContent = data.username;
           else if(userNicknameElement) userNicknameElement.textContent = user.email || 'User';
       }
   } catch (error) { console.error("Error checking/creating profile:", error); showError(`Profile Error: ${error.message}`); }
}
...
function setupAuthStateChangeListener() {
    ...
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
        console.log(`Auth State Change Event: ${_event}`, session ? `User: ${session.user?.id}` : 'No session'); // <-- ДОДАНО ЛОГ
        const user = session?.user ?? null;
         if (loginButton) {
            console.log("onAuthStateChange: Calling updateAuthStateUI..."); // <-- ДОДАНО ЛОГ
            updateAuthStateUI(user); // Оновлюємо UI
         } else { console.warn("onAuthStateChange: DOM not ready yet."); currentUser = user; }
        if (_event === 'SIGNED_IN' && user) {
            console.log("onAuthStateChange: SIGNED_IN detected, checking profile..."); // <-- ДОДАНО ЛОГ
           await checkAndCreateUserProfile(user);
           // Додатково оновити ім'я користувача в UI після перевірки/створення
            if(loginButton && userNicknameElement){
                 const { data: profile } = await supabaseClient.from('profiles').select('username').eq('id', user.id).single();
                 if(profile) userNicknameElement.textContent = profile.username || 'User';
            }
        }
        if (_event === 'SIGNED_OUT') {
            console.log("onAuthStateChange: SIGNED_OUT detected, resetting state..."); // <-- ДОДАНО ЛОГ
            stopTimer(); selectedDifficulty = 1; currentLeaderboardDifficulty = 1; currentLeaderboardTimeframe = 'all';
            if(gameAreaElement) gameAreaElement.style.display = 'none';
            if(resultAreaElement) resultAreaElement.style.display = 'none';
            if(difficultySelectionElement) difficultySelectionElement.style.display = 'none';
            if(leaderboardSectionElement) leaderboardSectionElement.style.display = 'none';
             console.log("onAuthStateChange: SIGNED_OUT state reset complete."); // <-- ДОДАНО ЛОГ
        }
    });
 ...
 async function checkInitialAuthState() {
     ...
     try {
         ...
         console.log("Initial session check complete.", currentUser ? `User: ${currentUser.id}` : "No user");
...
function startTimer() {
    ...
    timeLeftElement.textContent = timeLeft; console.log("Starting timer...");
...
function openLeaderboard() { console.log("Opening leaderboard..."); hideError(); if (!leaderboardSectionElement) { console.error("Cannot open leaderboard - element missing"); return; } if(gameAreaElement) gameAreaElement.style.display = 'none'; if(resultAreaElement) resultAreaElement.style.display = 'none'; if(difficultySelectionElement) difficultySelectionElement.style.display = 'none'; if(userStatusElement) userStatusElement.style.display = 'none'; if(authSectionElement && !currentUser) authSectionElement.style.display = 'none'; leaderboardSectionElement.style.display = 'block'; updateLeaderboard(); }
function closeLeaderboard() { console.log("Closing leaderboard..."); if (leaderboardSectionElement) leaderboardSectionElement.style.display = 'none'; updateAuthStateUI(currentUser); } // Show relevant UI after closing
...
function initializeApp() {
    ...
    if (!initializeDOMElements()) { // Initialize elements and check success
        console.error("CRITICAL: Failed to initialize DOM elements. App cannot start.");
        // handleCriticalError was called inside initializeDOMElements if it failed
        return;
    }
    setupAuthStateChangeListener(); // Setup listener AFTER elements are ready
    // Update UI based on initial check AFTER elements are ready
    ...
    if(resultAreaElement) resultAreaElement.style.display = 'none';
    if(difficultySelectionElement) difficultySelectionElement.style.display = 'none';
    if (showLeaderboardButton) showLeaderboardButton.style.display = 'inline-block'; // Show leaderboard button by default
    if (leaderboardSectionElement) leaderboardSectionElement.style.display = 'none'; // Hide leaderboard itself
}
