...
let currentUserProfile = null; // To cache the fetched profile
...
// Nickname elements
let userNicknameElement, editNicknameButton, editNicknameForm, nicknameInputElement, cancelEditNicknameButton; // save button is part of form
...
// Initialize DOM Elements (UPDATED to include nickname elements)
function initializeDOMElements() {
    ...
    userNicknameElement = document.getElementById('user-nickname'); // <-- ID змінено в HTML
    ...
    for (const key in elements) {
        if (!elements[key]) {
             const idName = key.replace('Element', '').replace('Button',''); // Approximate ID
             console.error(`Error: Could not find DOM element '${idName}'! Check HTML ID.`);
             allFound = false;
        }
    }
    if (!difficultyButtons || difficultyButtons.length !== 3) { /* ... */ allFound = false; }
    if (!leaderboardTimeFilterButtons || leaderboardTimeFilterButtons.length === 0) { /* ... */ allFound = false; }
    if (!leaderboardDifficultyFilterButtons || leaderboardDifficultyFilterButtons.length === 0) { /* ... */ allFound = false; }

    if (!allFound) { console.error("initializeDOMElements: Not all required elements found."); handleCriticalError("UI Error: Missing elements."); return false; }
    ...
    // Слухачі для редагування нікнейму
    userNicknameElement.addEventListener('click', showNicknameEditForm); // Клік на сам нікнейм
    editNicknameButton.addEventListener('click', showNicknameEditForm); // Клік на іконку
    editNicknameForm.addEventListener('submit', handleNicknameSave); // Обробка відправки форми
    cancelEditNicknameButton.addEventListener('click', hideNicknameEditForm); // Клік на кнопку Cancel
...
// Update UI state based on login status (uses userNicknameElement)
function updateAuthStateUI(user) {
   ...
   hideNicknameEditForm(); // Завжди ховати форму при зміні стану auth

   if (user) {
       currentUser = user;
       userNicknameElement.textContent = 'Loading...'; // Показуємо завантаження нікнейму
       userStatusElement.style.display = 'block';
       loginButton.style.display = 'none';
       showLeaderboardButton.style.display = 'inline-block';
       editNicknameButton.style.display = 'inline-block'; // Показуємо кнопку редагування
       ...
       currentUser = null;
       currentUserProfile = null; // Очищаємо кеш профілю
       ...
       if (editNicknameButton) editNicknameButton.style.display = 'none';
       stopTimer();
...
// Check/Create User Profile (UPDATED - fetches profile, updates UI, caches profile)
async function checkAndCreateUserProfile(user) {
   ...
   try {
       ...
       // Update UI with the final username
       ...
       currentUserProfile = null; // Clear cache on error
   }
}
...
function setupAuthStateChangeListener() {
    ...
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
        ...
        const user = session?.user ?? null;
         if (initializeDOMElements()) { // Ensure elements are found first
            updateAuthStateUI(user); // Update general UI
            if (_event === 'SIGNED_IN' && user) {
               await checkAndCreateUserProfile(user); // Load/Create profile & update nickname span
            }
...
async function checkInitialAuthState() {
    ...
    try {
        ...
        // If user exists, try to load profile right away
        if (currentUser) {
...
// Removed fetchInitialUserProfile as checkAndCreateUserProfile now handles initial load too

// ----- 5.5 Nickname Editing Functions -----
function showNicknameEditForm() {
    // Check if elements required for this specific function exist
    if (!currentUserProfile || !userNicknameElement || !editNicknameButton || !editNicknameForm || !nicknameInputElement) {
         console.error("Cannot show edit form - elements missing or user not loaded.");
         return;
    }
    console.log("Showing nickname edit form.");
    hideError();
    nicknameInputElement.value = userNicknameElement.textContent || ''; // Populate with current display name
    userNicknameElement.style.display = 'none';
    editNicknameButton.style.display = 'none';
    editNicknameForm.style.display = 'inline-block';
    nicknameInputElement.focus();
    nicknameInputElement.select(); // Select text for easy editing
}

function hideNicknameEditForm() {
    // Check elements needed here
    if (!userNicknameElement || !editNicknameButton || !editNicknameForm || !nicknameInputElement) {
         // Don't log error here, might be called when elements aren't expected yet (e.g., logged out)
         return;
    }
    // console.log("Hiding nickname edit form.");
    editNicknameForm.style.display = 'none';
    nicknameInputElement.value = '';
    // Only show these if user is logged in (check currentUser)
    if (currentUser) {
...
async function handleNicknameSave(event) {
    ...
    if (!currentUser || !nicknameInputElement || !supabaseClient || !currentUserProfile) {
         showError("Cannot save nickname now. Please try again.");
         return;
    }
    ...
    if (!newNickname || newNickname.length < 3 || newNickname.length > 25) {
        showError("Nickname must be 3-25 characters long.");
        return;
    }
    ...
    try {
        const { data: updatedData, error } = await supabaseClient
            ...
            .select('username') // Select the updated username
            .single(); // Expect one row back

        if (error) throw error;

        console.log("Nickname updated successfully:", updatedData);
        currentUserProfile.username = updatedData.username; // Update cached profile
        if (userNicknameElement) {
            userNicknameElement.textContent = updatedData.username; // Update UI display
        }
        hideNicknameEditForm(); // Hide the form
        showError("Nickname updated successfully!"); // Use showError for temporary success message
        setTimeout(hideError, 2000); // Hide message after 2s

        // Refresh leaderboard if visible
        if (leaderboardSectionElement && leaderboardSectionElement.style.display === 'block') {
            console.log("Refreshing leaderboard after nickname change...");
            updateLeaderboard();
        }
...
function endGame() { /* ... */ }
async function saveScore() { /* ... */ } // Includes GeoIP debug
...
function initializeApp() {
    ...
    // Update UI based on initial state (currentUser set by checkInitialAuthState)
