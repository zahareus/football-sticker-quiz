// script.js (Повний код з оновленням для intro text)
...
let difficultyButtons; // Тепер це контейнери .difficulty-option
...
let introTextElement; // <<<--- Новий елемент
...
function initializeDOMElements() {
    ...
    // --- Перевірка кнопок складності оновлена ---
    if (!difficultyButtons || difficultyButtons.length !== 3) { console.error(`Error: Found ${difficultyButtons?.length || 0} difficulty buttons, expected 3! Check selector '.difficulty-option .difficulty-button'.`); allFound = false; }
    // --------------------------------------------
...
// --- Оновлено: Керування видимістю introTextElement ---
function updateAuthStateUI(user) {
    if (!loginButton || !userStatusElement || !difficultySelectionElement || !userNicknameElement || !showLeaderboardHeaderButton || !landingPageElement || !introTextElement) { // Додано introTextElement
        console.warn("updateAuthStateUI: Core UI elements not ready yet!"); return;
    }
    const bodyElement = document.body; hideNicknameEditForm();
    ...
    if (introTextElement) introTextElement.style.display = 'none'; // Приховуємо і текст
    ...
    if (user) { // --- LOGGED IN ---
        ...
        // Показуємо вибір складності, якщо не відкрито лідерборд і не йде гра/результат
        if (leaderboardSectionElement?.style.display !== 'block' &&
            gameAreaElement?.style.display === 'none' &&
            resultAreaElement?.style.display === 'none')
        {
            showDifficultySelection(); // Ця функція тепер покаже і introText
        }
    } else { // --- LOGGED OUT ---
        ...
        stopTimer(); // Зупиняємо таймер, якщо він був активний

        // Показуємо лендінг (і текст), якщо не відкрито лідерборд
        if (leaderboardSectionElement?.style.display !== 'block') {
             if(landingPageElement) landingPageElement.style.display = 'flex';
             if(introTextElement) introTextElement.style.display = 'block'; // Показуємо текст
        }
    }
}
// ----------------------------------------------------------
...
function showDifficultySelection() {
    hideError();
    if (!difficultySelectionElement || !gameAreaElement || !resultAreaElement || !leaderboardSectionElement || !landingPageElement || !introTextElement ) { // Додано introTextElement
...
function endGame() {
    ...
    stopTimer();
    if(finalScoreElement) { finalScoreElement.textContent = currentScore; finalScoreElement.classList.remove('final-score-animated'); void finalScoreElement.offsetWidth; finalScoreElement.classList.add('final-score-animated'); }
    if(gameAreaElement) gameAreaElement.style.display = 'none';
    ...
    if(difficultySelectionElement) difficultySelectionElement.style.display = 'none';
    if(introTextElement) introTextElement.style.display = 'block'; // <<< Показуємо текст після гри
    saveScore();
}
// -------------------------------------------------------
...
function openLeaderboard() {
    ...
    if (!leaderboardSectionElement || !landingPageElement || !gameAreaElement || !resultAreaElement || !difficultySelectionElement || !introTextElement ) { // Додано introTextElement
        handleCriticalError("UI Error opening leaderboard."); return;
    }
    // Сховати всі інші основні блоки
    if(gameAreaElement) gameAreaElement.style.display = 'none';
    if(resultAreaElement) resultAreaElement.style.display = 'none';
    if(difficultySelectionElement) difficultySelectionElement.style.display = 'none';
    if(landingPageElement) landingPageElement.style.display = 'none';
    if(introTextElement) introTextElement.style.display = 'none'; // Ховаємо текст
    // Показати лідерборд
...
function closeLeaderboard() {
    ...
    if (leaderboardSectionElement) leaderboardSectionElement.style.display = 'none';
    // updateAuthStateUI визначить, що показати далі (лендінг або вибір складності + текст)
...
function initializeApp() {
    ...
    if(resultAreaElement) resultAreaElement.style.display = 'none';
    if (leaderboardSectionElement) leaderboardSectionElement.style.display = 'none';
    if (introTextElement) introTextElement.style.display = 'none'; // Початково ховаємо
}
