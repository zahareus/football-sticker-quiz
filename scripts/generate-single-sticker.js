...
import {
    ...
    cityOnly, stickerNoindexTag, selectTopRatedStickers, generateDescriptiveAltText, generateMultilingualAltText, generateStickerContextParagraph, stickerPageTitle, countryPageTitle, clubThumbHtml, generateMultilingualMeta,
...
/**
 * Generate wiki section HTML from wiki cache data (V2 layout)
 */
...
function generateNearbyStickers(currentSticker, nearbyStickers) {
    ...
    shown.forEach(s => {
        ...
        html += `\n<a href="/stickers/${s.id}.html" class="sticker-strip-item" title="${escapeHtml(s.clubName)}"><img src="${thumbUrl}" alt="${escapeHtml(s.clubName)}" data-sticker-id="${s.id}" width="100" height="100" loading="lazy" decoding="async"></a>`;
    });
    html += '\n</div>\n</div>';
    return html;
}
